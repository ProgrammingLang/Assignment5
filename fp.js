/**
 * This small module supports the Functional Programming style by providing
 * standard accessors and mutators for a list data structure, as well as 
 * common list-processing functions (namely, append and reverse) and
 * classic higher-order functions (such as compose, curry, map, filter and
 * reduce). Most of the other functions in this module deal with numbers
 * (e.g., add, sub, mul, div, pow, isNumber, isZero) but other functions
 * (e.g., isEq and the list accessors and constructors) will work fine when 
 * manipulating lists of other primitives values (e.g., strings, booleans)
 * or nested lists thereof.
 * 
 * @module fp
...
/** makeList takes in 0 or more primitive values and returns a single list
...
var makeArithmeticOp = function (name,f) {
    return function (a,b) {
    if (name === "add") { console.log("Add"); }
    if (name === "mul") { console.log("Mul"); }
...
/** mul takes in two numbers n1 and n2 and returns n1 * n2
    @function 
    @param n1 {number} - any finite number    
    @param n2 {number} - any finite number
    @alias module:fp.mul
...
/** div takes in two numbers n1 and n2 and returns n1 / n2
    ...
    @param n2 {number} - any finite number
    @alias module:fp.div
    @return {number} the result of dividing the second number into the first
    one
...
/** min takes in two numbers n1 and n2 and returns the largest of the two
    @function 
    @param n1 {number} - any finite number    
    @param n2 {number} - any finite number
    @alias module:fp.min
...
/** map takes in a function f of one argument and a list aList, and returns the 
    list that contains the all of the return values when f is applied to the
    elements of aList
    @param f {Function} - any function of one argument
    @param aList {list} - any list of values in the domain of f
    @alias module:fp.map
    @return {list} the list of return values of the input function when applied
    to all of the elements in the input list
